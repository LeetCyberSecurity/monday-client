...
async def test_post_request_success(client_instance):
    ...
    with patch.object(client_instance, '_execute_request', new_callable=AsyncMock, return_value=(mock_response, mock_headers)):
        with patch.object(client_instance, '_ensure_version_set', new_callable=AsyncMock):
